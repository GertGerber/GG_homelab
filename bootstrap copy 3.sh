...
write_env_file() {
  ...
  cat > "$env_file" <<EOF
# Autogenerated by bootstrap on $(date -u +'%Y-%m-%dT%H:%M:%SZ')
export PKG=$(printf '%q' "$PKG")
export PKG_UPDATE=$(printf '%q' "$PKG_UPDATE")
export PKG_INSTALL=$(printf '%q' "$PKG_INSTALL")
export ENVIRONMENT=$(printf '%q' "$ENVIRONMENT")
EOF
  success "Wrote env helpers: $env_file"
...
main() {
  ...
  devider
  # minimal sanity
  # for c in curl bash tar; do have_cmd "$c" || die "Missing required command: $c"; done
  detect_pkgmgr
  devider
  write_env_file
  devider
  ...
  printf 'Next: run your follow-up scripts (e.g., ./scripts/setup.sh) after sourcing %s\n' "$WORKDIR/.bootstrap_env"
}
